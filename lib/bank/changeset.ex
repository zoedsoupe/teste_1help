defmodule Bank.Changeset do
  ...
  An example input could be "matheus PESSANHA", which would return "Zoey Pessanha"
  """
  ...
  @doc """
  Validates a phone both on mobile and landline formats
  """
  def validate_phone(changeset, field) do
    validate_change(changeset, field, fn _, value ->
      if String.match?(value, @mobile_format) do
        []
      else
        [{field, "not a valid phone"}]
      end
    end)
  end

  @doc """
  Validates an e-mail format
  """
  def validate_email(changeset, field) do
    validate_change(changeset, field, fn _, value ->
      if String.match?(value, @email_format) do
        []
      else
